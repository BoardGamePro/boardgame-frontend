...
export default function GameCard({ gameInfo, viewMode = 'grid' }) {
  ...
  return (
    <div
      className={`flex w-full rounded-[10px] bg-white shadow-[0px_4px_4px_0px_rgba(142,141,208,0.16)] ${
        viewMode === 'grid'
          ? 'h-[441px] flex-col'
          : 'flex-row gap-[20px] p-[20px]'
      }`}
    >
      <div
        className={`relative flex-shrink-0 overflow-hidden ${
          viewMode === 'grid'
            ? 'h-[200px] w-full rounded-t-[10px]'
            : 'h-[210px] w-[210px] rounded-t-none'
        }`}
      >
        {img && (
      ...
      <div
        className={`flex h-full flex-col gap-[15px] ${
          viewMode === 'grid' ? 'p-[20px_10px]' : 'w-full'
        }`}
      >
